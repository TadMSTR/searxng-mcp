...
- **HTTP transport session leak bounded** — sessions were removed only on `transport.onclose`, which an agent killed mid-turn never fires, leaking the transport + its MCP server on the long-lived process. Added an idle-session sweep (`HTTP_SESSION_IDLE_TIMEOUT_MS`, default 10 min) plus an LRU hard-cap backstop (`HTTP_MAX_SESSIONS`, default 256). Evictions are logged.
