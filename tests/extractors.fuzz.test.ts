...
// Property tests over jsdom are an order of magnitude slower than example
// tests, and vitest's default 5s per-test timeout is tuned for the latter.
// Three of these passed locally at 1.7-2.8s and TIMED OUT on CI, where the
// runner is roughly 2-3x slower — a green local run said nothing about it.
//
// Raised per-test rather than globally in vitest.config.ts: a slack timeout
// everywhere would hide a genuinely hung test among the other 1,200. Reducing
// numRuns was the alternative and was rejected — these run counts are what take
// `source === "json_ld"` from 0 to 78 hits per 400, and trimming them would
// quietly walk back the coverage this build just measured.
const JSDOM_PROPERTY_TIMEOUT = 30_000;
...
describe("extractTitle — total, and always a non-empty string", () => {
  it(
    ...
    JSDOM_PROPERTY_TIMEOUT,
  );

  it(
    "is deterministic — the same DOM twice gives the same title",
    () => {
      fc.assert(
        fc.property(html, url, (h, u) => {
          const a = domOf(h, u);
...
describe("scanJsonLd / extractJsonLdArticle — total, declared shape", () => {
  it(
    "scanJsonLd never throws and returns booleans/strings only",
    () => {
      fc.assert(
        fc.property(html, url, (h, u) => {
          const dom = domOf(h, u);
  ...
  it(
    "extractJsonLdArticle returns null or a genuinely populated article",
    () => {
      fc.assert(
        fc.property(html, url, (h, u) => {
          const dom = domOf(h, u);
  ...
  it(
    "HTML with no ld+json block is reported absent by both",
    () => {
      fc.assert(
        fc.property(
          fc
...
describe("runReadability — total", () => {
  it(
    "never throws; returns null or a result with a string text",
    () => {
      fc.assert(
        fc.property(html, url, (h, u) => {
  ...
  it(
    "never throws on a raw arbitrary string as HTML",
    () => {
      fc.assert(
...
describe("preferReadability — decision table holds for all inputs", () => {
  it(
    "null readability is never preferred",
    () => {
      fc.assert(
  ...
  it(
    "below the 500-char floor, any readability result wins; above it, only a longer one does",
    () => {
      fc.assert(
        fc.property(
...
describe("postExtract — total, and honours maxChars", () => {
  it(
    "never throws, always returns the declared shape",
    () => {
      fc.assert(
        fc.property(
          html,
  ...
  it(
    "the returned text never exceeds maxChars",
    () => {
      fc.assert(
        fc.property(
          html,
  ...
  it(
    "never returns an empty title, whatever the page carries",
    () => {
      fc.assert(
        fc.property(
          html,
  ...
  it(
    "source === 'json_ld' implies jsonLdPresent",
    () => {
      fc.assert(
        fc.property(
          html,
